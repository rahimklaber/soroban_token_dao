...
fn set_for_votes(env: &Env, prop_id: u32, amount: i128){
    env
    .storage()
    .set(DataKey::ForVotes(prop_id), amount)
}
...
fn set_against_votes(env: &Env, prop_id: u32, amount: i128){
    env
    .storage()
    .set(DataKey::AgainstV(prop_id), amount)
}
...
fn set_abstain_votes(env: &Env, prop_id: u32, amount: i128){
    env
    .storage()
    .set(DataKey::AbstainV(prop_id), amount)
}
...
pub fn votes_counts(env: &Env, prop_id: u32) -> VotesCount{
    ...
    VotesCount { v_for: for_votes, v_against: against_votes, v_abstain: abstain_votes }
}